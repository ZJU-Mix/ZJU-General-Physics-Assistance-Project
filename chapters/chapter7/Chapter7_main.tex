\chapter[热力学]{\itr{Thermodynamics}{热力学}}
    \section[导论]{\itr{Introduction}{导论}}
        ...
        于是，我们引出\itr{Statistical Physics}{统计物理学}与\itr{Themiodynamics}{热力学}\linebreak 这两个研究超多粒子体系的分支。
    ...
    \section[分子动理论]{\itr{Kinetic Theory of Molecules}{分子动理论}}
        ...
        \subsection[能量均分原理]{\itr{Equipartition Theorem}{能量均分原理}}
            ...
            \end{Itemize}
            \ \newline 
            有了自由度的概念，根据大量的实验事实总结有如下关系成立(\refleaftext{prove7.11})：
            \begin{equation}
                U = \frac{i}{2}Nk_{B}T
            \end{equation}
        ...
        \subsection[麦克斯韦分布]{\itr{Maxwell Distribution}{麦克斯韦分布}}
            ...
            \itr{Maxwell Distribution}{麦克斯韦分布}给出了理想气体的速率分布规律，它给出的速率分布函数如下(\refleaftext{prove7.10})：
            \begin{equation}
    ...
    \section[热力学第一定律]{\itr{First Law of Thermodynamics}{热力学第一定律}}
        \subsection[热与功]{\itr{Heat and Work}{热与功}\footnote{关于本小节中等容热容和等压热容的定义，有一点需要说明：有些资料，如普通化学中，等容热容和等压热容的定义与本书中定义有所出入。这些材料另用了摩尔等容热容$C_{V,m}$和摩尔等压热容$C_{p,m}$来表示体积(压强)一定的情况下，$1\mathrm{mol}$物质上升单位温度所需吸收的热量，而等容热容$C_V$和等压热容$C_p$则定义为体积(压强)一定的情况下，所研究物质上升单位温度所需吸收的热量。本书采用了路欣老师2023学年春夏学期某一节ppt内的定义。事实上，老师的多节ppt内关于该符号的定义也并未统一。}}
            \begin{Itemize}
                ...
                \end{equation}
                这是\itr{Fourier's Law}{傅里叶定律}，其中$\kappa_{t}$表示介质的热传导率，与介质种类有关；$A$为截面面积；\\[1ex]
                ...
                并规定系统对外做功取正。功也是过程函数。                
            \end{Itemize}
            (理想气体热容的证明见\refleaftext{prove7.3})
        ...
        \subsection[过程介绍]{\itr{Process Introduction}{过程介绍}\labelroot{chapter7_process_introduction}}
    ...
    \section[热力学第二定律]{\itr{Second Law of Thermodynamics}{热力学第二定律}}
        \subsection[热机]{\itr{Heat Engine}{热机}}
            ...
            \end{center}
            使用正卡诺循环的热机称为\itr{Carnot Heat Engine}{卡诺热机}，其热机效率为(\refleaftext{prove7.8})：
            \begin{equation}
                e = 1 - \frac{T_{c}}{T_{h}}
                \nonumber
            \end{equation}
        ...
        \subsection[熵]{\itr{Entropy}{熵}}
            ...
            由于等温，$\Delta U=0$；那么由热力学第一定律，自然有$Q=0$。\\[1ex]但是计算它的熵变，这个值为$\Delta S = nR\ln\dfrac{V_2}{V_1}>0$ (见\refleaftext{example7.6}(b))。
            众所周知，这个\\[1ex]过程是不可逆的，也就是说，那个$\Delta S<0$的逆过程是无法发生的。
            这正是\linebreak\itr{Second Law of Thermodynamics}{热力学第二定律}所表明的。
    ...
    \section[信息]{\itr{Information}{信息}}
        ...
        然后再来了解一下\itr{Cross Entropy}{交叉熵}\mgnote{该知识不作要求。}。对于某事件，假设其有两种概率分布为$p(x)$与$q(x)$，
        则其交叉熵为：
        \begin{equation}
