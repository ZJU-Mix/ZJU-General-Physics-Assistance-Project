    ...
    \begin{solution}[{\color{plainred}\large Pressure}\\In \itr{state-of-the-art}{最先进的} \itr{vacuum systems}{真空系统}, pressures as low as $1.00\times10^{-9}\rm{Pa}$ are being attained. 
    ...
    \end{solution}
    \newpage
    \begin{solution}[{\color{plainred}\large Idea Gas}\\The root-mean-square 
    ...
    \end{solution}
    \newpage
    \begin{solution}[{\color{plainred}\large The Van Der Waals Gas}\\The van der Waals equation of state is as follows:
        \begin{equation*}
        ...
        \end{equation*}
    \end{solution}
    \begin{solution}[{\color{red}\large Heat and Work}\\An ideal \itr{diatomic}{双原子的} gas, in a \itr{cylinder}{圆柱体} with a movable \itr{piston}{活塞}, undergoes the rectangular cyclic process shown
    ...
    \end{solution}
    \newpage
    \begin{solution}[{\color{plainred}\large Carnot Cycle}\\For a van der Waals gas, its equation of state implies a phase transition between
        ...
        \end{equation*}
    \end{solution}
    \begin{solution}[{\color{plainred}\large Entropy}\\ Consider the Carnot cycle operating with a hot and cold heat baths whose temperatures are $T_h$ and
        $T_c$ ($T_h > T_c$), respectively. Working \itr{substance}{材料} means the gas in the engine, and we consider a gas in
        general for the working substance.\\
        (a) Suppose the amount of heat exchange during the \itr{isothermal process}{等温过程} with the hot (cold) heat bath is
        ...
        熵变则利用熵是状态函数与自由膨胀始态与终态温度相等的特性，用等温过程(\refleaftext{chapter7_process_introduction})连接始态与终态，熵变为：
        \begin{equation*}
